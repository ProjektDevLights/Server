...
export class ControlService {
  ...
  private callback = async (name: string) => {
    ...
    try {
      this.tcpService.sendData(this.utilsService.genJSONforEsp({ command: "off" }), esp.ip);
  ...
  async scheduleOff(id: string, data: ScheduleOffDto): Promise<StandartResponse<Light>> {
    const oldDoc: EspDocument = await this.databaseService.getEspWithId(id);

    const minute = data.minute;

    let date = new Date();
    date = moment(date).add(minute, "minutes").toDate();

    let cronPattern = `${date.getMinutes().toString()} ${date.getHours().toString()} * * *`;

    try { this.cronService.addCron(`off-${oldDoc.uuid}`, cronPattern, this.callback); }
    catch {
      throw new NotAcceptableException("There is already one timer for turning off this light running");
    }

    return {
      message: `The light will turn off in ${minute} ${minute > 1 ? "minutes" : "minute"}!`,
